...
//        }
    ...
    $scope.loadItems = function() {
        ...
        $http.get('/api/jwt/ws/crud?type=' + $scope.backendObject + "&action=shared&filter=sharedbyall")
...
//                        alert('An error has occurred: '+ e.message + ' - The application will not function correctly. Please contact the developer!');
                        $console.error('shared.js loadItems(): An error has occurred: '+ e.message + ' - The application will not function correctly. Please contact the developer!');
                    }
                }
                $("#loadingTable").hide();
            }
...
//                    $scope.movie_title2 = response.data.data.items[0].title;
                    ...
                    if(response && response.data && response.data.data && response.data.data.items[0] && response.data.data.items[0].player) {
                        $scope.movie_url2 = response.data.data.items[0].player["default"];
                    }
        ...
        if(!offline) {
            ...
            .success(function(response, status, headers, config){
                ...
                if(response.id > 0) {
                    ...
                    $scope.item.owner = response.owner == $scope.userId?"You":response.owner;
        ...
        if(!offline) {
            ...
            .success(function(response, status, headers, config){
                ...
                if(response.id === $scope.item.id) {
                    ...
                    $scope.originalItem.owner = response.owner == $scope.userId?"You":response.owner;
