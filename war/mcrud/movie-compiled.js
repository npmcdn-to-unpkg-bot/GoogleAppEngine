...
//log,
$timeout, dateFilter, $location, $sce) {
    ...
    $scope.serviceCheck = function (page) {
        if (typeof page.server_max !== 'undefined' && typeof page.server_number !== 'undefined') {
            ...
            if (page.number > 0 && page.server_number !== page.number && page.server_number !== 0) {
                //page.server_number === 0 is the same as the client side page 1
                alert('page.server_number [' + page.server_number + '] page.number [' + page.number + ']');
                throw new Error("Page number " + page.number + " is out of sync with the server" + "s page (" + page.server_number + "). The page number should be in sync/the same!");
            }
            if (page.max > 0 && page.server_max !== page.max) {
                throw new Error("Maximum per page " + page.max + " is out of sync with the server" + "s maximum (" + page.server_max + "). The max count per page should be in sync/the same!");
            }
    ...
    $scope.loadItems = function () {
        ...
        $http.get(gCacheProxy + '/ws/crud?type=' + $scope.backendObject + "&origin=" + location.hostname + "&origin=" + location.hostname + "&origin=" + location.hostname + "&aid=" + gAppId + "&uid=" + uid + "&maxPerPage=" + $scope.page.max + "&pageNumber=" + $scope.page.number).success(function (data, status1, headers, config) {
            ...
            if (typeof status !== 'undefined') {
                //TODO workaround to suppress the weird error! :(
                ...
                } else if (status.indexOf(App.gaej_server_error_msg) > -1) {
    ...
    $scope.loadItemsJustForHashtag = function () {
        ...
        $http.get(gCacheProxy + '/ws/crud?type=' + $scope.backendObject + "&origin=" + location.hostname + "&aid=" + gAppId + "&uid=" + uid + "&maxPerPage=" + $scope.page.max + "&pageNumber=" + $scope.page.number).success(function (data, status1, headers, config) {
            ...
            try {
                $scope.templates = [{ name1: 'Shuffle Mode', url: '/html/channelshuffle.html?username=' + $.url().param('username') + '&logintype=' + $.url().param('logintype') + '&type=' + gPlayNowShuffle }, { name1: 'All In Sequence', url: '/html/channelall.html?username=' + $.url().param('username') + '&logintype=' + $.url().param('logintype') + '&type=' + gPlayNowAllInSeq }];
    ...
    $scope.search = function () {
        ...
        if ($scope.savedPageMax && $scope.savedPageNumber) {
            //do not override if it is initial load (page just bootstrapped)
            ...
            if (item !== undefined) {
                ...
                for (var attr in item) {
                    if (item.hasOwnProperty(attr)) {
                        //http://stackoverflow.com/questions/18451795/undefined-var-in-ie8-javascript-jquery
    ...
    $scope.sort_by = function (newSortingOrder) {
        ...
        if ($scope.reverse) $('th.' + newSortingOrder + ' i').removeClass().addClass('icon-chevron-up');else $('th.' + newSortingOrder + ' i').removeClass().addClass('icon-chevron-down');
    };
    ...
    $scope.serializeSearch = function () {
        return $scope.searchResults = [{ movie_thumbnail: escapeJson($scope.movie_thumbnail), movie_title: escapeJson($scope.movie_title), movie_url: escapeJson($scope.movie_url) }, { movie_thumbnail: escapeJson($scope.movie_thumbnail2), movie_title: escapeJson($scope.movie_title2), movie_url: escapeJson($scope.movie_url2) }, { movie_thumbnail: $scope.movie_thumbnail3, movie_title: escapeJson($scope.movie_title3), movie_url: escapeJson($scope.movie_url3) }];
    };
    ...
    $scope.scheduleItem = function (form) {
        ...
        if (form.$valid) {
            //care only if there is no form validation error e.g. due to required etc c.f. http://scotch.io/tutorials/javascript/angularjs-form-validation
    ...
    $scope.selectNextPage = function (page) {
        ...
        if (page.number == 1) {
            //has to put a check as the request might be accidentally called from non-paginated request :(
            page.number = 2;
        }
...
.config(['$controllerProvider', '$httpProvider', function ($controllerProvider, $httpProvider) {
    //alert('test');
...
}]).controller('MovieController', ['$scope', '$filter', '$http', '$rootScope',
//'loglevel',
'$timeout', 'dateFilter', '$location', '$sce', MovieController //1.3 does not allow global and we need it to be as angular.bootstrap needs a global function
])
